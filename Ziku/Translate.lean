...
mutual
  ...
  partial def translateExpr (e : Expr) : TranslateM Producer :=
    ...
    | .lam pos param isCov body => do
      if isCov then
        -- Covalue parameter: \~k => e
        -- Translates to μk. ⟨⟦e⟧ | k⟩
        let bodyP ← translateExpr body
        return .mu pos param (.cut pos bodyP (.covar pos param))
      else
        -- Normal lambda: \x => e
        -- ⟦λx.t⟧ = cocase {ap(x; α) ⇒ ⟨⟦t⟧ | α⟩}
        let α ← freshCovar
    ...
    | .app pos fn arg isCov => do
      if isCov then
        -- Application to covalue: f(~k)
        -- ⟦f ~k⟧ = μα. ⟨⟦f⟧ | ⟦k⟧_as_consumer⟩
        -- Since we currently only support variables/labels as covalues
        match arg with
        | .var _ name =>
          let α ← freshCovar
          let fnP ← translateExpr fn
          return .mu pos α (.cut pos fnP (.covar pos name))
        | _ => throw $ .notImplemented pos "complex covalue arguments"
      else
        -- Normal application
    ...
    | .goto pos value continuation => do
      ...
      | .var _ labelName =>
        ...
        if inScope then
          ...
          -- If not a label, assume it's a first-class covalue (covar in IR)
          return .mu pos β (.cut pos valueP (.covar pos labelName))
      | _ =>
        -- Complex expression for continuation
        throw $ .notImplemented pos "complex continuation in goto"
