...
open Ziku (Ident BinOp Builtin Lit)
...
-- Translate built-in function to Scheme expression
-- Returns the Scheme code that applies the builtin to its arguments
def translateBuiltinApp (b : Builtin) (argCodes : List String) : String :=
  match b with
  | .strLen =>
    -- (string-length s)
    s!"(string-length {argCodes[0]!})"
  | .strAt =>
    -- (string-ref s i)
    s!"(string-ref {argCodes[0]!} {argCodes[1]!})"
  | .strSub =>
    -- (substring s start (+ start len))
    s!"(substring {argCodes[0]!} {argCodes[1]!} (+ {argCodes[1]!} {argCodes[2]!}))"
  | .strToInt =>
    -- (string->number s)
    s!"(string->number {argCodes[0]!})"
  | .intToStr =>
    -- (number->string n)
    s!"(number->string {argCodes[0]!})"
  | .runeToStr =>
    -- (string c)
    s!"(string {argCodes[0]!})"
  | .intToRune =>
    -- (integer->char n)
    s!"(integer->char {argCodes[0]!})"
  | .runeToInt =>
    -- (char->integer c)
...
partial def translateStatementM : Statement → GenM String
  ...
  | .builtin _ b args c => do
    -- builtin(p̄; c) → (c (builtin-op arg1 arg2 ...))
    -- If any arg is a μ, we need to evaluate it first
    let cCode ← translateConsumerM c
    -- Check if any args are μ-abstractions that need evaluation
    let hasThunks := args.any fun p => match p with | .mu _ _ _ => true | _ => false
    if hasThunks then
      -- Generate a chain of evaluations for μ-abstractions
      -- This is simplified: evaluate all args to fresh vars first
      let indices := List.range args.length
      let argVars ← indices.mapM fun i => freshVar s!"arg{i}"
      -- Each binding returns (code, numParensToClose)
      -- μ args open 2 parens: ((lambda...) (lambda ...)), let args open 1 paren
      ...
      let builtinCall := translateBuiltinApp b argVars
      let closingParens := String.ofList (List.replicate totalParens ')')
      pure (String.intercalate "" argBindings ++ s!"({cCode} {builtinCall})" ++ closingParens)
    else
      -- All args are values
      let argCodes ← args.mapM translateProducerM
