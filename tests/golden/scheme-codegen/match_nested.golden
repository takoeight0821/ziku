...
(define (ziku-main halt)
  (let ((_alpha0 halt)) (let ((x (list 'Just 10))) (let ((_alpha1 _alpha0)) (let ((%p x)) (if (and (vector? %p) (eq? (vector-ref %p 0) 'ziku-thunk)) ((vector-ref %p 1) (lambda (%v) (let ((%tag (car %v))) (cond ((eq? %tag 'Just) (let ((n (list-ref %v 1))) (let ((_alpha2 _alpha1)) (_alpha2 (* n 2))))) ((eq? %tag 'Nothing) (let ((%p 0)) (if (and (vector? %p) (eq? (vector-ref %p 0) 'ziku-thunk)) ((vector-ref %p 1) _alpha1) (_alpha1 %p)))))))) ((lambda (%v) (let ((%tag (car %v))) (cond ((eq? %tag 'Just) (let ((n (list-ref %v 1))) (let ((_alpha2 _alpha1)) (_alpha2 (* n 2))))) ((eq? %tag 'Nothing) (let ((%p 0)) (if (and (vector? %p) (eq? (vector-ref %p 0) 'ziku-thunk)) ((vector-ref %p 1) _alpha1) (_alpha1 %p))))))) %p)))))))
