...
(define (ziku-main halt)
  (let ((_alpha0 halt)) (let ((r (list (cons 'a 42)))) (let ((_alpha1 _alpha0)) (let ((%p r)) (if (and (vector? %p) (eq? (vector-ref %p 0) 'ziku-thunk)) ((vector-ref %p 1) (lambda (%rec) (let* ((%is-dispatch (and (vector? %rec) (eq? (vector-ref %rec 0) 'ziku-dispatch))) (%v (cond (%is-dispatch ((vector-ref %rec 1) 'a)) ((procedure? %rec) (%rec 'a)) (else (cdr (assq 'a %rec)))))) (if (and (vector? %v) (eq? (vector-ref %v 0) 'ziku-thunk)) ((vector-ref %v 1) _alpha1) (_alpha1 %v))))) ((lambda (%rec) (let* ((%is-dispatch (and (vector? %rec) (eq? (vector-ref %rec 0) 'ziku-dispatch))) (%v (cond (%is-dispatch ((vector-ref %rec 1) 'a)) ((procedure? %rec) (%rec 'a)) (else (cdr (assq 'a %rec)))))) (if (and (vector? %v) (eq? (vector-ref %v 0) 'ziku-thunk)) ((vector-ref %v 1) _alpha1) (_alpha1 %v)))) %p)))))))
